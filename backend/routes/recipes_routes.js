...
router.post("/", (req, res) => {
  ...
  // const checkName = "SELECT * FROM recipes WHERE recipeName = ?";
  // db.pool.query(checkName, [recipeName], (checkErr, checkResult) => {
  //   if (checkErr) {
  //     console.error("Error executing SQL query:", checkErr);
  //     return res
  //       .status(500)
  //       .send("Database error during checking for duplicates");
  //   }
  //   console.log("Check result:", checkResult);
  //   if (checkResult.length > 0) {
  //     return res
  //       .status(400)
  //       .send("Recipe already exists. Please choose a different name.");
  //   }

  // add new recipe if name is unique
